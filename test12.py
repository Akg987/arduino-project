...
class TeacherManager(QMainWindow):
    ...
    def edit_teacher(self):
        ...
        for entry in teacher_entries:
            if entry["day"] in current_day_time:
                entry["name"] = name
    ...
    def delete_teacher(self):
        ...
        # دریافت کد استاد از سطر انتخاب‌شده
        teacher_id_item = self.teacher_table.item(selected_row, 0)
        if not teacher_id_item:
            QMessageBox.warning(self, "خطا", "اطلاعات استاد انتخاب شده معتبر نیست.")
            return
        ...
        # دریافت اطلاعات روز و ساعت از ستون مربوطه
        day_time_item = self.teacher_table.item(selected_row, 3)
        if not day_time_item:
            QMessageBox.warning(self, "خطا", "اطلاعات روز و ساعت انتخاب شده معتبر نیست.")
            return

        # استخراج اطلاعات روز و ساعت
        selected_day_time = day_time_item.text()

        # پیدا کردن ورودی مرتبط و حذف آن
        if teacher_id in self.teachers:
            updated_entries = []
            for entry in self.teachers[teacher_id]:
                # بررسی روز و ساعت مرتبط
                day = entry.get("day", "")
                times = ', '.join(entry.get("times", []))
                if f"{day}: {times}" != selected_day_time:
                    updated_entries.append(entry)

            # به‌روزرسانی لیست ورودی‌های استاد
            if updated_entries:
                self.teachers[teacher_id] = updated_entries
            else:
                # اگر لیست خالی شد، کل استاد حذف شود
                del self.teachers[teacher_id]

        # به‌روزرسانی جدول
        self.update_table()
        QMessageBox.information(self, "حذف موفق", f"اطلاعات مربوط به روز و ساعت '{selected_day_time}' برای استاد با کد {teacher_id} حذف شد.")
