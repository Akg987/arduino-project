...
class TeacherManager(QMainWindow):
    def __init__(self):
        ...
        self.time_slots = ['7:30,9:30', '9:30, 11:30', '12:00,14:00', '14:00,16:00', '16:00,18:00', '18:00-20:00','20:00-22:00']
        ...
        self.teacher_table.setHorizontalHeaderLabels(["کد استاد", "نام استاد", "کلاس", "روزها و زمان"])
    ...
    def add_teacher(self):
        teacher_id = self.code_input.text().strip()
        teacher_name = self.name_input.text().strip()
        teacher_class = self.class_input.text().strip()

        if not teacher_id or not teacher_name or not teacher_class:
            QMessageBox.warning(self, "خطا", "لطفاً تمامی اطلاعات استاد را وارد کنید.")
            return

        selected_days_and_times = []

        for day, (day_checkbox, time_checkboxes) in self.days_time_checkboxes.items():
            if day_checkbox.isChecked():
                selected_times = [time_checkbox.text() for time_checkbox in time_checkboxes if time_checkbox.isChecked()]
                if selected_times:
                    selected_days_and_times.append({"day": day, "times": selected_times})

        if not selected_days_and_times:
            QMessageBox.warning(self, "خطا", "حداقل یک روز و زمان را انتخاب کنید.")
            return

        # ثبت اطلاعات استاد در ساختار جدید
        if teacher_id not in self.teachers:
            self.teachers[teacher_id] = []

        for entry in selected_days_and_times:
            self.teachers[teacher_id].append({
                "name": teacher_name,
                "class": teacher_class,
                "day": entry["day"],
                "times": entry["times"]
        ...
        self.update_table()  # به‌روزرسانی جدول اساتید
        self.clear_inputs()  # پاک کردن ورودی‌ها

    def edit_teacher(self):
            current_item = self.teacher_table.currentItem()
            if not current_item:
                    QMessageBox.warning(self, "خطا", "لطفاً یک استاد را انتخاب کنید.")
                    return

            # دریافت شماره سطر از سلول انتخاب‌شده
            selected_row = current_item.row()
            old_code = self.teacher_table.item(selected_row, 0).text()
            info = self.teachers.get(old_code, None)
        ...
        # دریافت اطلاعات جدید
            new_name, ok_name = QInputDialog.getText(self, "ویرایش نام استاد", "نام استاد:", QLineEdit.Normal, info["name"])
            new_code, ok_code = QInputDialog.getText(self, "ویرایش کد استاد", "کد استاد:", QLineEdit.Normal, old_code)
            new_class, ok_class = QInputDialog.getText(self, "ویرایش کلاس استاد", "کلاس استاد:", QLineEdit.Normal, info["class"])

            if ok_name and ok_code and ok_class:
            # بروزرسانی روزها و ساعت‌ها
                days_info = {}
            for day, (day_checkbox, time_checkboxes) in self.days_time_checkboxes.items():
                if day_checkbox.isChecked():
                    days_info[day] = [time.text() for time in time_checkboxes if time.isChecked()]

            # ذخیره اطلاعات جدید
            if old_code != new_code:
                del self.teachers[old_code]  # حذف استاد قدیمی
            self.teachers[new_code] = {
                "name": new_name,
                "class": new_class,
                "days": days_info,
            }
            ...
            QMessageBox.information(self, "موفقیت", "اطلاعات استاد با موفقیت ویرایش شد.")
    ...
    def delete_teacher(self):
        ...
        code = self.teacher_table.item(selected_row, 0).text()
        confirmation = QMessageBox.question(self, "تأیید حذف", f"آیا مطمئن هستید که می‌خواهید استاد {code} را حذف کنید؟", QMessageBox.Yes | QMessageBox.No)
        
        if confirmation == QMessageBox.Yes:
            del self.teachers[code]
            self.save_teachers_to_file()  # ذخیره تغییرات در فایل
            self.update_table()  # به‌روزرسانی جدول بعد از حذف استاد
            QMessageBox.information(self, "موفقیت", "استاد با موفقیت حذف شد.")

    def update_table(self):
        self.teacher_table.setRowCount(0)  # پاک کردن جدول
        for teacher_id, entries in self.teachers.items():
            for info in entries:  # پیمایش لیست اطلاعات هر استاد
                row_position = self.teacher_table.rowCount()
                self.teacher_table.insertRow(row_position)
                self.teacher_table.setItem(row_position, 0, QTableWidgetItem(teacher_id))
                self.teacher_table.setItem(row_position, 1, QTableWidgetItem(info["name"]))
                self.teacher_table.setItem(row_position, 2, QTableWidgetItem(info["class"]))
                day_time = f"{info['day']}: {', '.join(info['times'])}"
                self.teacher_table.setItem(row_position, 3, QTableWidgetItem(day_time))
    ...
    def process_data(self, line):
        try:
            ...
            parts = line.split('*')
            if len(parts) == 2:
                class_code = parts[0].strip()  # کلاس (E101)
                teacher_code = parts[1].strip()  # کد استاد (1010)
                ...
                if teacher_code in self.teachers:
                    ...
                    print(f"روزهای موجود برای استاد {teacher_code}: {teacher['days']}")

                    # بررسی تطابق روز و زمان
                    if current_day in teacher['days'] and any(
                        self.is_time_in_slot(current_time, slot) for slot in teacher['days'][current_day]
                    ):
                        # چک کردن زمان آخرین حضور
                        last_time = self.last_attendance.get(teacher_code)
                        if last_time:
                            elapsed_time = now - last_time
                            if timedelta(hours=1) <= elapsed_time <= timedelta(hours=1,minutes=30):  # تغییر در محدوده زمان
                    ...
                    self.attendance_table.setItem(row_position, 2, QTableWidgetItem(teacher['name']))
                    self.attendance_table.setItem(row_position, 3, QTableWidgetItem(teacher['class']))
