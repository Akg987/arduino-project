...
class TeacherManager(QMainWindow):
    def __init__(self):
        ...
        self.time_slots = ['7:30-9:30', '9:30-11:30', '12:00-14:00', '14:00-16:00', '16:00-18:00', '18:00-20:00','01:00-03:00']
       ...
       # تنظیم تایمر
        ...
        self.timer.timeout.connect(self.check_absences)
        self.timer.start(2 * 60 * 60 * 1000)  # هر 60 ثانیه یک بار چک کند
        ...
        self.attendance_table.setColumnCount(7)  # تعداد ستون‌ها را افزایش دهید
        self.attendance_table.setHorizontalHeaderLabels(["زمان", "کد", "نام", "کلاس", "روز", "نام درس", "وضعیت"])  # اضافه کردن ستون وضعیت
        ...
        try:
            ...
            if len(parts) == 2:
                ...
                if teacher_code in self.teachers:
                        ...
                        if record['day'] == current_day:
                            ...
                            if any(self.is_time_in_slot(current_time, slot) for slot in record['times']):
                                if record['class'] == class_code:
                                    ...
                                    status = "🔴"  # وضعیت دایره قرمز برای کلاس نادرست
                                break
                    ...
                    self.attendance_table.setItem(row_position, 2, QTableWidgetItem(record.get('name', "نام استاد")))
                    ...
                    self.attendance_table.setItem(row_position, 5, QTableWidgetItem(course_name))
                    self.attendance_table.setItem(row_position, 6, QTableWidgetItem(status))  # ستون وضعیت
    ...
    def check_absences(self):
        ...
        for teacher_code, teacher_data in self.teachers.items():
            attendance_found = False
            present_in_other_class = False

            for record in teacher_data:
                if record['day'] == current_day and any(self.is_time_in_slot(current_time, slot) for slot in record['times']):
                    if teacher_code in self.last_attendance:
                        attendance_found = True
                    else:
                        present_in_other_class = True
                    break

            if not attendance_found:
                status = "❌❌❌"
                course_name = record.get('course', "نام درس")
                row_position = self.attendance_table.rowCount()
                self.attendance_table.insertRow(row_position)
                self.attendance_table.setItem(row_position, 0, QTableWidgetItem(now.strftime("%Y-%m-%d %H:%M:%S")))
                self.attendance_table.setItem(row_position, 1, QTableWidgetItem(teacher_code))
                self.attendance_table.setItem(row_position, 2, QTableWidgetItem(record.get('name', "نام استاد (غیاب)")))
                self.attendance_table.setItem(row_position, 3, QTableWidgetItem(record['class']))
                self.attendance_table.setItem(row_position, 4, QTableWidgetItem(current_day))
                self.attendance_table.setItem(row_position, 5, QTableWidgetItem(course_name))
                self.attendance_table.setItem(row_position, 6, QTableWidgetItem(status))
                self.save_attendance_to_file()

            elif present_in_other_class:
                status = "🔴"
                course_name = record.get('course', "نام درس")
                row_position = self.attendance_table.rowCount()
                self.attendance_table.insertRow(row_position)
                self.attendance_table.setItem(row_position, 0, QTableWidgetItem(now.strftime("%Y-%m-%d %H:%M:%S")))
                self.attendance_table.setItem(row_position, 1, QTableWidgetItem(teacher_code))
                self.attendance_table.setItem(row_position, 2, QTableWidgetItem(record.get('name', "نام استاد (حاضر در کلاس دیگر)")))
                self.attendance_table.setItem(row_position, 3, QTableWidgetItem(record['class']))
                self.attendance_table.setItem(row_position, 4, QTableWidgetItem(current_day))
                self.attendance_table.setItem(row_position, 5, QTableWidgetItem(course_name))
                self.attendance_table.setItem(row_position, 6, QTableWidgetItem(status))
